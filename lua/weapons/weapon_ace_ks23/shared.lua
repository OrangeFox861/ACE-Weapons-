...
SWEP.Purpose = " 23mm slavik disintegrator (caliber 5mm because no one wants to catch 12mm pellets)"
SWEP.Spawnable = true
...
SWEP.ViewPunchAmount =10--grees to punch the view upwards each shot - does not actually move crosshair, just a visual effect


--Spread (aimcone) settings--
SWEP.BaseSpread = 2 --First-shot random spread, in degrees
...
SWEP.ViewModel = "models/ace+/v_ks23.mdl"
SWEP.ViewModelFOV			= 50
SWEP.WorldModel = "models/ace+/v_ks23.mdl"
...
pos = pos + ang:Right() * -1 + ang:Up() * -11 + ang:Forward() * 10
return pos, ang
end
...
if CLIENT then
	...
	function SWEP:DrawWorldModel()
		...
		if (IsValid(_Owner)) then
			...
			local offsetVec = Vector(0, -4, -1)
			local offsetAng = Angle(35, 10, 180)
