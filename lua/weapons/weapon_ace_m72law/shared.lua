...
--Main settings--
SWEP.FireRate = 0.22 --Rounds per second

SWEP.Primary.ClipSize = 1
SWEP.Primary.DefaultClip = 2
SWEP.Primary.Automatic = false
SWEP.Primary.Ammo = "RPG_Round"
SWEP.Primary.Sound = "acf_extra/cannons/mortars-launchers/mortar_60mm.wav"
...
SWEP.ViewModel = "models/ace+/v_law.mdl"
SWEP.WorldModel = "models/ace+/w_law.mdl"
SWEP.HoldType = "rpg"
...
pos = pos + ang:Right() * 2 + ang:Up() * -5 + ang:Forward() * 10
return pos, ang
end
...
function SWEP:InitBulletData()
	...
	self.BulletData.Caliber = 6.6
	self.BulletData.PropLength = 5 --Volume of the case as a cylinder * Powder density converted from g to kg
	self.BulletData.ProjLength = 70 --Volume of the projectile as a cylinder * streamline factor (Data5) * density of steel
	self.BulletData.Data5 = 2700 --He Filler or Flechette count
...
function SWEP:PrimaryAttack()
	...
	if (IsFirstTimePredicted() or game.SinglePlayer()) and owner:IsPlayer() then
		self:GetOwner():ViewPunch(Angle(-7, 0, 0))
	end
	...
	self:SetNextPrimaryFire( CurTime() + (1 / self.FireRate) )
		...
		if SERVER then
			...
			local MDat = {
				...
				Pos = owner:GetShootPos() + owner:GetAimVector() * 500,
				...
				MotorSound = "acf_extra/cannons/50mm-107mm/cannon_76to99/17pdr.wav",
