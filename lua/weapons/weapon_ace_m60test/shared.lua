...
SWEP.PrintName = "m60 test"
SWEP.Category = "ACE Weapons+"
SWEP.SubCategory = "Machine Guns"
SWEP.Purpose = "heavy NATO bratatata"
SWEP.Spawnable = true
SWEP.Slot = 2
SWEP.SlotPos = 1
SWEP.Author = " "
...
--Main settings--
SWEP.FireRate = 9 --Rounds per second
...
SWEP.HeatReductionRate = 1655 --Heat loss per second when not firing
SWEP.HeatPerShot = 22.04 --Heat generated per shot
SWEP.HeatMax = 22.04 --Maximum heat - determines max rate at which recoil is applied to eye angles
...
SWEP.AngularRecoil = 22.04	--Amount of angular recoil
...
SWEP.ViewPunchAmountSide = 0.3
SWEP.ViewPunchAmountRoll = 0.2
SWEP.AccurateCrosshair = true

--Spread (aimcone) settings--
SWEP.BaseSpread = 0 --First-shot random spread, in degrees
SWEP.MaxSpread = 0.2 --Maximum added random spread from heat value, in degrees
					--If HeatMax is 0 this will be ignored and only BaseSpread will be taken into account (AT4 for example)
SWEP.MovementSpread = 4.5 --Increase aimcone to this many degrees when sprinting at full speed
SWEP.UnscopedSpread = 0 --Spread, in degrees, when unscoped with a scoped weapon

--Model settings--
SWEP.ViewModelFlip = false
SWEP.ViewModel = "models/ace+/v_m60.mdl"
SWEP.WorldModel = "models/ace+/w_m60.mdl"
SWEP.HoldType = "ar2"
...
pos = pos + ang:Right() * 3 + ang:Up() * -5 + ang:Forward() * 10
return pos, ang
end

SWEP.CarrySpeedMul			= 0.57 --WalkSpeedMult when carrying the weapon
...
sound.Add( {
    ...
    volume = 0.5,
    level = 80,
    sound       = "Weapons/lmg/m60/reload1.wav"
} )
...
sound.Add( {
    ...
    volume = 0.5,
    level = 80,
    sound       = "Weapons/lmg/m60/reload2.wav"
} )
sound.Add( {
    name = "CA.M60.Handle",
    channel = CHAN_STATIC,
    volume = 0.5,
    level = 80,
    sound       = "Weapons/lmg/m60/reload3.wav"
} )
sound.Add( {
    name = "CA.M60.Select",
    channel = CHAN_STATIC,
    volume = 0.5,
    level = 80,
    sound       = "Weapons/lmg/m60/select.wav"
} )
